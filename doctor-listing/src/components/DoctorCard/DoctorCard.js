...
const DoctorCard = ({ doctor }) => {
  ...
  // Normalize fees for display
  const feesNumber = Number(doctor.fees);
  const showFees = !isNaN(feesNumber) && feesNumber > 0;

  return (
    <div
      className={styles.doctorCard}
      data-testid="doctor-card" // <-- Added here
    ...
    >
      <div className={styles.doctorProfile}>
        ...
        <div className={styles.doctorInfo}>
          <h3 className={styles.doctorName} data-testid="doctor-name">
      ...
      <div className={styles.doctorActions}>
        <div className={styles.consultationFee}>
          ...
          <span className={styles.feeAmount} data-testid="doctor-fee">
            {showFees ? `₹ ${feesNumber}` : 'Not specified'}
          </span>
        </div>
        ...
        <button className={styles.appointmentButton} onClick={handleBookAppointment}>
          Book Appointment
        </button>
      </div>
    </div>
